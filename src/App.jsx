...
import {
  getRandomNumberFromLenghtOfData,
  getArrayOfKeywords
} from "./helpers/Util";
...
class App extends Component {
  ...
  constructor(props) {
    ...
    this.handleAccessibiltySelectionChange = this.handleAccessibiltySelectionChange.bind(
      this
    );
  ...
  loadData() {
    ...
    for (let i = 0; i < getRandomNumberFromLenghtOfData(this.state.data); i++) {
      tableData.push(
        this.createData(getArrayOfKeywords(this.state.data), this.id)
      );
    }
    return tableData;
  }
  ...
  render() {
    ...
    return (
      <div className="App">
            ...
            <section>
              <div>
                ...
                <select
                  ...
                  className=""
                  ref="dropdown"
                  onChange={this.handleDataSelectionChange}>
                  {" "}
                  {this.state.availableLanguages.map((item, index) => (
                    <option className="" key={index + "_" + item} value={item}>
                      {" "}
                      {item}{" "}
                    </option>
                  ))}{" "}
              ...
              <div>
                ...
                <select
                  ...
                  className=""
                  ref="accessibilty"
                  onChange={this.handleAccessibiltySelectionChange}>
                  {" "}
                  {this.state.availableThemes.map((item, index) => (
                    <option className="" key={index + "_" + item} value={item}>
                      {" "}
                      {item}{" "}
                    </option>
              ...
              <div className="invertedControl">
                <label htmlFor="showWordsHighlighted">
                  {" "}
                  Enter words to highlight:{" "}
                </label>{" "}
                <input
                  id="showWordsHighlighted"
                  ref="wordsToHighlight"
                  onChange={this.updateHighlightedWords}
                />{" "}
        ...
        <SplitPane
          split="vertical"
          defaultSize={window.innerWidth >> 1}
          onChange={size =>
            this.setState({
              splitSize: size
            })
          ...
          <div>
            <div className={accClass}>
              <WordCloud
                data={data}
                width={this.state.splitSize}
                height={this.wordCloudHeight}
                scoreKey={"freq"}
                highlightWords={this.state.arrHighlightWords}
                  ...
                  {this.state.tableData.map((item, index) => {
                    ...
                    return (
                      <TableRow
                        ...
                        style={rowStyle}
                        onClick={() =>
                          this.updateHighlightedWordsFromTable(item.keywords)
                        }>
                        <TableCell> {item.id} </TableCell>{" "}
                        <TableCell> {item.keywords.toString()} </TableCell>{" "}
                      </TableRow>
                    );
                  })}{" "}
                </TableBody>{" "}
